...
  
  // Importante: Los objetos match ya incluyen photoURL
...
            p2: group[j],
            winner: null,
            photoURL: null, // NUEVO: URL de la foto
...
    });
    ...
    // Attach Event Listeners to photo upload input (NUEVO)
    document.querySelectorAll('.match-photo-upload').forEach(input => {
...
    const isCompleted = match.winner !== null;
    const photoURL = match.photoURL || null; // NUEVO: Obtener URL de la foto
...
                    <label class="px-3 py-1 bg-gray-100 text-gray-700 text-sm rounded-md cursor-pointer hover:bg-gray-200 transition duration-150">
                        📸 ${photoURL ? 'Cambiar Foto' : 'Añadir Foto (Opcional)'}
                        <input type="file" accept="image/*" class="hidden match-photo-upload" data-match-id="${match.id}" ${isCompleted ? 'disabled' : ''}>
                    </label>
                    <div id="photo-preview-${match.id}">
                        ${photoURL ? `<a href="${photoURL}" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm font-semibold">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline-block" viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd" d="M4 2a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2V4a2 2 0 00-2-2H4zm12 12H4l4-4 3 3 5-5v8z" clip-rule="evenodd" />
                            </svg> Ver Foto
                        </a>` : ''}
                    </div>
                </div>
...
            button.addEventListener('click', handleAddSet);
        });
        // Re-attach photo upload listener (NEW)
        cardContainer.querySelectorAll('.match-photo-upload').forEach(input => {
            input.addEventListener('change', handlePhotoUpload);
        });
    }
}
...
// ---------------------------
// PHOTO UPLOAD LOGIC (NEW)
// ---------------------------

async function handlePhotoUpload(event) {
    // Check if cloud mode and storage are available before proceeding
    if (!window.isCloudMode || !window.storage) {
        showStatus("⚠️ Cloud Storage no está disponible. No se puede subir la foto. Activa Firebase.", "red");
    ...
    if (match.winner !== null) {
        showStatus("⚠️ No se puede cambiar la foto de un partido completado. Deshaga el resultado primero.", "orange");
        event.target.value = null; 
        return;
    }

    showStatus("⏳ Subiendo foto... por favor espera.", "orange");
    
    try {
        // 2. Create Firebase Storage reference
        const storageRef = window.ref(window.storage, `tournaments/${window.userId}/match_photos/${matchId}_${Date.now()}_${file.name}`);
        
        // 3. Upload the file
        const snapshot = await window.uploadBytes(storageRef, file);
        
        // 4. Get the public URL
        const photoURL = await window.getDownloadURL(snapshot.ref);

        // 5. Update the match object with the URL
        match.photoURL = photoURL;

        // 6. Save and re-render
        saveData(true);
        reRenderMatchCard(match);
        showStatus("✅ Foto subida y guardada en el partido.", "green");

    } catch (error) {
        console.error("Error uploading photo:", error);
        showStatus(`❌ Error al subir la foto: ${error.message}`, "red");
...
    [[1, rank1, '🏆 Campeón'], [2, rank2, '🥈 Subcampeón'], [3, rank3, '🥉 3er Lugar'], [4, rank4, '4to Lugar']]
